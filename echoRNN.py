...
W = tf.Variable(np.random.rand(state_size+1, state_size), dtype=tf.float32) # [4+1,4]
...
for current_input in inputs_series:
    ...
    input_and_state_concatenated = tf.concat(1, [current_input, current_state])  # Increasing number of columns = [5,5]

    next_state = tf.tanh(tf.matmul(input_and_state_concatenated, W) + b)  # Broadcasted addition [5,5] * [4+1,4] + [1,4] = [5,4]
